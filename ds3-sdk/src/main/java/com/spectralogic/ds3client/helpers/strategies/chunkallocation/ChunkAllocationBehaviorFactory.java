...
public final class ChunkAllocationBehaviorFactory {
    ...
    public static RetryBehavior makeRetryBehavior(final int maxNumIterations) {
        RetryBehavior retryBehavior = new InfiniteRetryBehavior();

        if (maxNumIterations >= 1) {
            retryBehavior = new MaxIterationRetryBehavior(maxNumIterations, new RetryFailureCallback() {
                @Override
                public void onFailure(final int iterationNumber) throws IOException {
    ...
    public static DelayBehavior makeDelayBehavior(final int numSecondsToDelay,
                                                  final DelayBehavior.DelayBehaviorCallback delayBehaviorCallback) {
